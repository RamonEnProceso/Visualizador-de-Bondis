...
const listaBondis: Bondi[]  = bondis; 

const arrayToObjects = (list:number[]) : Bondi[] =>{
    const arrList = list.map((n)=>{
        const bondi = listaBondis.find(bondi => bondi.linea === n);
        if(bondi){
            return bondi
        }
        return null
...
const BondisDisplay = ({list}:{list:number[]}) => {
    ...
    const arrList : Bondi[] = arrayToObjects(list);
